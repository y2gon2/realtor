...
### 1.2 서비스 생명주기 다이어그램 — 보고서 한 건이 만들어지는 전체 흐름

아래는 사용자가 주소를 입력하고 보고서를 받기까지, 8개 서비스가 **시간 순서대로** 어떻게 연결되는지를 보여줍니다.
...
│  [사용자 브라우저]                                                                                │
│       │                                                                                         │
│       │ ① "마포래미안푸르지오 84타입 보고서 만들어줘"                                                 │
│       │    (HTTP POST /api/v1/reports)                                                           │
│       ▼                                                                                         │
│  [Next.js 프론트엔드] (:3000)                                                                    │
│       │                                                                                         │
│       │ ② API 요청 전달                                                                          │
│       │    (HTTP POST → go-api:8080)                                                            │
│       ▼                                                                                         │
│  [Go API 서버] (:8080)                                                                           │
│       │                                                                                         │
│       ├── ③ JWT 토큰 검증 (인증 확인)                                                             │
│       │                                                                                         │
│       ├── ④ PostgreSQL에 보고서 레코드 생성                                                       │
│       │      INSERT INTO reports (address_input, purpose, status)                                │
│       │      VALUES ('마포래미안푸르지오', '매매_실거주', 'pending')                                  │
│       │      → report_id = 'abc123' 반환                                                        │
│       │        │                                                                                │
│       │        ▼                                                                                │
│       │   [PostgreSQL] (:5432)                                                                  │
│       │                                                                                         │
│       └── ⑤ Redis Streams에 작업 발행                                                            │
│              XADD report_jobs * report_id abc123 address "마포래미안푸르지오" purpose "매매_실거주"   │
│                │                                                                                │
│                ▼                                                                                │
│           [Redis] (:6379)  ← 작업이 큐에 들어감                                                   │
│                                                                                                 │
│       → 사용자에게 즉시 응답: { report_id: "abc123", status: "pending" }                           │
│       → 프론트엔드가 SSE 연결 시작: GET /api/v1/reports/abc123/progress (EventStream)              │
│                                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────────────────────────────────────┐
│ Phase 2: 백그라운드 보고서 생성 (Python Worker)                                                    │
├─────────────────────────────────────────────────────────────────────────────────────────────────┤
│                                                                                                 │
│  [Python Worker]                                                                                │
│       │                                                                                         │
│       │ ⑥ Redis Streams에서 작업 가져오기                                                         │
│       │    XREADGROUP GROUP workers consumer1 COUNT 1 BLOCK 5000 STREAMS report_jobs >           │
│       │      │                                                                                  │
│       │      ▼                                                                                  │
│       │   [Redis] (:6379)  ← 작업을 가져감 (큐에서 제거되지 않고 "처리 중"으로 마킹)                  │
│       │                                                                                         │
│       ├── ⑦ 주소 정규화                                                                          │
│       │    "마포래미안푸르지오" → Kakao Geocoding API 호출 (외부 HTTP)                               │
│       │    → 법정동코드, PNU, 좌표(lat/lng), 도로명주소 획득                                        │
│       │                                                                                         │
│       ├── ⑧ 13개 외부 API 병렬 호출 (asyncio.gather)                                             │
│       │    ├── 실거래가 API (data.go.kr) × 12개월                                                │
│       │    ├── 건축물대장 API (data.go.kr)                                                       │
│       │    ├── 토지이용규제 API (data.go.kr)                                                     │
│       │    ├── KOSIS 통계 API                                                                   │
│       │    ├── 학교정보 API                                                                      │
│       │    └── 뉴스 API (네이버/빅카인즈)                                                         │
│       │    (모두 외부 HTTP 호출 — Docker 네트워크를 통해 인터넷 접근)                                 │
│       │                                                                                         │
│       ├── ⑨ 진행 상태 실시간 전송 (매 단계마다)                                                    │
│       │      PUBLISH report:progress:abc123 '{"step":"api_collect","progress":30}'               │
│       │        │                                                                                │
│       │        ▼                                                                                │
│       │   [Redis Pub/Sub] (:6379)                                                               │
│       │        │                                                                                │
│       │        ▼                                                                                │
│       │   [Go API] ← SUBSCRIBE report:progress:abc123                                           │
│       │        │                                                                                │
│       │        ▼                                                                                │
│       │   [SSE → 프론트엔드] → 사용자 화면에 진행 바 업데이트 (30%)                                 │
│       │                                                                                         │
│       ├── ⑩ 정책 룰엔진 계산 (순수 Python, 외부 호출 없음)                                         │
│       │    ├── 취득세: 주택 수/가격/지역 기반 세율 적용                                             │
│       │    ├── 양도소득세: 보유기간/거주기간별 시나리오                                              │
│       │    └── LTV/DSR 대출 한도: 규제지역/용도별 계산                                              │
│       │                                                                                         │
│       ├── ⑪ RAG 벡터 검색 (전문가 인사이트)                                                       │
│       │      "마포구 부동산 전망" → 벡터 변환 요청                                                  │
│       │        │                                                                                │
│       │        ▼                                                                                │
│       │   [Embedding 서버] (:8001)  ← 텍스트를 1024차원 벡터로 변환                                │
│       │        │                                                                                │
│       │        ▼                                                                                │
│       │   [Qdrant] (:6333)  ← 벡터 유사도 검색 → 상위 3개 관련 문서 반환                            │
│       │                                                                                         │
│       ├── ⑫ 7개 섹션 LLM 생성 (최대 3개 병렬)                                                    │
│       │      ├── Section 1: 입지 분석                                                            │
│       │      ├── Section 2: 시세/시장 분석 + 차트 4개                                             │
│       │      ├── Section 3: 법률/규제 분석                                                       │
│       │      ├── Section 4: 투자 수익 분석 (룰엔진 결과 포함)                                      │
│       │      ├── Section 5: 일조/조망 분석                                                       │
│       │      ├── Section 6: 리스크 분석                                                          │
│       │      └── Section 7: 미래 가치                                                            │
│       │      │                                                                                  │
│       │      ▼                                                                                  │
│       │   [Claude API] (외부 HTTP — Anthropic 서버)                                              │
│       │      각 섹션당 ~10-30초, 총 ~60-90초                                                     │
│       │                                                                                         │
│       │   (매 섹션 완료 시 ⑨ 진행 상태 전송 반복)                                                  │
│       │   → "Section 3/7 완료" → 프론트엔드 진행 바 (60%)                                         │
│       │                                                                                         │
│       ├── ⑬ 차트 생성 (matplotlib) — 4개 PNG 이미지                                              │
│       │      ├── 5년 시세 추이 그래프                                                             │
│       │      ├── 최근 3개월 거래량                                                                │
│       │      ├── 비교 매물 가격                                                                   │
│       │      └── 가격+거래량 이중축 콤보                                                           │
│       │                                                                                         │
│       ├── ⑭ PDF 조립 + 파일 업로드                                                               │
│       │      7개 섹션 Markdown + 차트 이미지 → PDF 생성                                           │
│       │        │                                                                                │
│       │        ▼                                                                                │
│       │   [MinIO] (:9000)  ← PUT realtor-reports/2026/04/report_abc123.pdf                      │
│       │                    ← PUT realtor-reports/2026/04/chart_price_abc123.png (×4)             │
│       │                                                                                         │
│       └── ⑮ 완료 처리                                                                           │
│              ├── PostgreSQL에 상태 업데이트                                                       │
│              │     UPDATE reports SET status='completed', pdf_url='...' WHERE id='abc123'        │
│              │       │                                                                          │
│              │       ▼                                                                          │
│              │   [PostgreSQL] (:5432)                                                            │
│              │                                                                                  │
│              ├── Redis Streams 작업 확인 (처리 완료 마킹)                                          │
│              │     XACK report_jobs workers {message_id}                                        │
│              │       │                                                                          │
│              │       ▼                                                                          │
│              │   [Redis] (:6379)                                                                │
│              │                                                                                  │
│              └── 최종 진행 상태 전송                                                              │
│                    PUBLISH report:progress:abc123 '{"step":"done","progress":100}'               │
│                                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────────────────────┘
...
│  [Go API 서버]                                                                                   │
│       │                                                                                         │
│       │ ⑯ SSE로 "완료" 이벤트 수신 → 프론트엔드에 전달                                             │
│       │                                                                                         │
│       ▼                                                                                         │
│  [Next.js 프론트엔드]                                                                            │
│       │                                                                                         │
│       │ ⑰ 보고서 페이지로 이동 (/report/abc123)                                                  │
│       │    GET /api/v1/reports/abc123 → 보고서 메타데이터 + 섹션 내용                              │
│       │                                                                                         │
│       │ ⑱ 차트 이미지 + PDF 다운로드                                                             │
│       │    GET http://minio:9000/realtor-reports/2026/04/chart_*.png                             │
│       │    GET http://minio:9000/realtor-reports/2026/04/report_abc123.pdf                       │
│       │      │                                                                                  │
│       │      ▼                                                                                  │
│       │   [MinIO] (:9000) → 파일 반환                                                           │
│       │                                                                                         │
│       ▼                                                                                         │
│  [사용자 브라우저]                                                                                │
│       → 7개 섹션 보고서 + 차트 4개 + PDF 다운로드 버튼 표시                                         │
│       → 총 소요 시간: ~60-120초                                                                  │
│                                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────────────────────┘
```
...
| # | 서비스 | 생명주기 내 역할 | 관여 단계 |
|---|--------|---------------|----------|
| 1 | **Next.js 프론트엔드** | 사용자 인터페이스. 요청 전송 + 진행 바 표시 + 결과 렌더링 | ①②⑰⑱ |
| 2 | **Go API 서버** | 인증, DB 쓰기, 작업 큐 발행, SSE 진행 상태 중계 | ③④⑤⑯ |
| 3 | **PostgreSQL** | 보고서 메타데이터 영구 저장 (상태, URL, 섹션 내용) | ④⑮ |
| 4 | **Redis** | 작업 큐 (Streams) + 실시간 진행 상태 (Pub/Sub) + 캐시 | ⑤⑥⑨⑮ |
| 5 | **Python Worker** | 핵심 연산 — API 수집, 룰엔진, LLM 호출, 차트, PDF 생성 | ⑥~⑮ |
| 6 | **Embedding 서버** | 텍스트 → 벡터 변환 (RAG 검색용) | ⑪ |
| 7 | **Qdrant** | 벡터 유사도 검색 (전문가 인사이트 검색) | ⑪ |
| 8 | **MinIO** | 생성된 PDF/차트 파일 저장 + 다운로드 URL 제공 | ⑭⑱ |
