...
> **중요:** Phase 4의 `codes/report/`, `codes/api/`, `codes/rules/`, `codes/generation/` 코드는 **재사용**한다.
> Python Worker는 새로 작성하지만, 그 내용은 Redis Streams 컨슈머 + 기존 `ReportOrchestrator` 호출만 담당하는 얇은 래퍼다.
...
Sprint 2 (1.5주): 보고서 비동기 처리
  ...
  ├─ codes/realtor-ai-worker/ 작성 (XREADGROUP 컨슈머)
...
### 4.2 Python Worker (얇은 래퍼)

| 항목 | 선택 |
|------|------|
| 위치 | `codes/realtor-ai-worker/` (rag 트리 내, 별도 git 레포 아님) |
| 진입점 | `python -m realtor_ai_worker` (`__main__.py`) |
| Redis 클라이언트 | `redis-py` 5.x (이미 `python-worker` 컨테이너에 설치) |
| Postgres 클라이언트 | `psycopg[binary]` 3.x |
| 보고서 생성 | **기존 `codes/report/orchestrator.ReportOrchestrator.generate()` 그대로 호출** |
| LLM | 기존 `codes/generation/llm_client.create_llm_client()`, `LLM_BACKEND=cli` |

> Worker는 import 경로상 `codes/`를 PYTHONPATH로 사용하여 기존 모듈을 그대로 활용한다.
> docker-compose의 `python-worker` 서비스가 이미 `PYTHONPATH=/workspace/codes`로 설정되어 있다.
...
## 5. Phase 4 코드와의 연동 지점

| Phase 4 자산 | 위치 | 5-1에서의 사용 |
|-------------|------|-------------|
| `ReportOrchestrator.generate()` | [codes/report/orchestrator.py:73](../../../../codes/report/orchestrator.py#L73) | Worker가 메시지 1건당 1회 호출 |
| `_notify(step, detail)` 콜백 | [codes/report/orchestrator.py:94](../../../../codes/report/orchestrator.py#L94) | Worker가 Redis PUBLISH로 매핑 |
| `create_llm_client()` | [codes/generation/llm_client.py:220](../../../../codes/generation/llm_client.py#L220) | `LLM_BACKEND=cli`로 자동 분기, 코드 변경 없음 |
| `KakaoMapClient` | [codes/api/clients/kakao.py](../../../../codes/api/clients/kakao.py) | Worker는 그대로, Go API는 `internal/api/kakao.go`로 즉시 응답용 재구현 |
| `RealTransactionClient` | [codes/api/clients/real_transaction.py](../../../../codes/api/clients/real_transaction.py) | Worker가 그대로 사용 |
| `BuildingRegisterClient` | [codes/api/clients/building_register.py](../../../../codes/api/clients/building_register.py) | Worker가 그대로 사용 |
| `run_all()` 룰엔진 | [codes/rules/engine.py:83](../../../../codes/rules/engine.py#L83) | Worker가 그대로 사용 |
| `params.yaml` | [codes/report/config/params.yaml](../../../../codes/report/config/params.yaml) | Worker만 읽음. Go API는 자체 환경변수로 관리 |
| 인터뷰 4단계 흐름 | [codes/report/interview.py:32](../../../../codes/report/interview.py#L32) | Go API가 REST로 노출 (`/address/resolve` + `/reports` 본문에 purpose 포함) |
